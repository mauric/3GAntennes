...
%           v(phi)'inv(Rib)v(phi)
%           
% Interference qui se rapproche a la source
...
Phi1 = 32*pi/180 % angle dï¿½incidence du premier interfï¿½rent
...
RSI1 = linspace(300,-2,4); % rapport de puissance (en dB) entre le signal utile et lï¿½interfï¿½rent n?1
RSI2 = linspace(300,-2,4); % rapport de puissance (en dB) entre le signal utile et lï¿½interfï¿½rent n?2
...
k = 2;
    ...
    Rib = ((M/RSI2(k)).*v1*v1'+(M/RSI2(k)).*v2*v2'+(1/RSB).*eye(M));
    Rib_inv = inv(Rib);
...
[MatriceR,MatriceS,x,BinaireIn,PenteSCurve] = GeneSignaux(Phis,Phi1,Phi2,RSB,RSI1(k),RSI2(k));
...
h = get(0,'children');
for i=length(h):-1:1
  saveas(h(i), ['exo43_' num2str(length(h)+1-i)], 'png');
end
